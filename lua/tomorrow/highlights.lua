local c = require('tomorrow.colors')
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('tomorrow.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "tomorrow.nvim" })
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.tomorrow_config.highlights) do
